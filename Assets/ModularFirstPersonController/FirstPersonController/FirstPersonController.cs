...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void HeadBob()
    {
        if (isWalking)
        {
            float bobFrequency = bobSpeed;

            if (isSprinting)
            {
                timer += Time.deltaTime * (bobFrequency + sprintSpeed);
            }
            else if (isCrouched)
            {
                timer += Time.deltaTime * (bobFrequency * speedReduction);
            }
            else
            {
            ...
            joint.localPosition = new Vector3(jointOriginalPos.x + Mathf.Sin(timer) * bobAmount.x, jointOriginalPos.y + bobY, jointOriginalPos.z + Mathf.Sin(timer) * bobAmount.z);

            if (bobY < -0.03f && !playedStepSound)
            {
    ...
    {
        // Plays footstep sound when player collides with a collider
        if (other.gameObject.tag == "Wood")
        {
            footstep = FootstepManager.Instance.PlayFootstep(FootstepType.Wood);
        }
        else if (other.gameObject.tag == "Stone")
        {
            footstep = FootstepManager.Instance.PlayFootstep(FootstepType.Stone);
        }
    }
}
